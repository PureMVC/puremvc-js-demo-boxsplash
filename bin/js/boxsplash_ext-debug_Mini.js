Ext.namespace("Boxsplash");Boxsplash.ApplicationFacade=Ext.extend(Puremvc.patterns.Facade,{constructor:function(key){Boxsplash.ApplicationFacade.superclass.constructor.call(this,key);
...
}});Ext.apply(Boxsplash.ApplicationFacade,{STARTUP:"Startup",RETRIEVE_CONFIG_OPTION:"retrieveConfigOption",TOGGLE_START_STOP:"toggleStartStop",ANIMATION_STATE_CHANGED:"animationStateChanged",getInstance:function(key){if(!Puremvc.patterns.Facade.hasCore(key)){new Boxsplash.ApplicationFacade(key);
}var retVal=Puremvc.patterns.Facade.getInstance(key);return retVal;}});Ext.namespace("Boxsplash.controller");
